(define (problem roversEnMarte)
  ...
  (:goal
    (and
      (forall (?b - base) (and (or (not(esAsentamiento ?b)) (= (personasEnBase ?b) 0)) (or (not(esAlmacen ?b)) (= (suministrosEnBase ?b) 0)))) ; almacen implica suministrosEnBase=0, traducido logicamente      (= (personasEnCirculacion) 0)
      (= (suministrosEnCirculacion) 0)
      (= (personasEnCirculacion) 0)
    )
  )
  ;(:metric minimize (+ (penalizacion-total) (* 10 (total-cost))))
